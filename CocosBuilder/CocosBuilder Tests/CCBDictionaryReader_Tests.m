...
- (void)testNodeGraphFromDocumentDict_supportFileTypeCocosBuilder
{
    NSDictionary *document = @{
                               CCB_DICTIONARY_KEY_FILETYPE : @"CocosBuilder",
                               CCB_DICTIONARY_KEY_FILEVERSION : @(kCCBDictionaryFormatVersion),
...
- (void)testNodeGraphFromDocumentDict_fileVersionHigherThanSpriterBuilderSupports
{
    ...
    XCTAssertNil(node);
    XCTAssertNotNil(error);
    XCTAssertEqual(error.code, SBCCBReadingErrorVersionHigherThanCocosBuilderSupport);
...
- (void)testNodeGraphFromDocumentDict_bmfont
{
 ...
 {
 CCNode *node = [CCBDictionaryReader nodeGraphFromDocumentData:[self loadCCBFile:@"test_ccbreader_physics_distance_joint"] parentSize:CGSizeMake(1024.0, 1024.0) error:NULL];
 }
 
 - (void)testNodeGraphFromDocumentDict_physics_pivot_joint
 {
 CCNode *node = [CCBDictionaryReader nodeGraphFromDocumentData:[self loadCCBFile:@"test_ccbreader_physics_pivot_joint"] parentSize:CGSizeMake(1024.0, 1024.0) error:NULL];
 }
 
 - (void)testNodeGraphFromDocumentDict_physics_spring_joint
 {
 CCNode *node = [CCBDictionaryReader nodeGraphFromDocumentData:[self loadCCBFile:@"test_ccbreader_physics_spring_joint"] parentSize:CGSizeMake(1024.0, 1024.0) error:NULL];
 }
 
 - (void)testNodeGraphFromDocumentDict_physicsnode
 {
 CCNode *node = [CCBDictionaryReader nodeGraphFromDocumentData:[self loadCCBFile:@"test_ccbreader_physicsnode"] parentSize:CGSizeMake(1024.0, 1024.0) error:NULL];
 }
