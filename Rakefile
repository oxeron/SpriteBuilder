...
TEMPLATE_PROJECT = File.join PROJECT_ROOT, "Support", "#{TEMPLATE_FILENAME}.cocosbuilder"
...
DEFAULT_PRODUCT_NAME="CocosBuilder"
...
file "Generated/cocos2d_version.txt" => 'CocosBuilder/libs/cocos2d-iphone/VERSION' do |task|
    puts "Generating #{task.name()}..."

    `cp CocosBuilder/libs/cocos2d-iphone/VERSION #{task.name()}`
end

def write_xcconfig
    f = File.open("CocosBuilder/Resources/Version.xcconfig","w")
...
namespace :build do
    desc "Build (only) CocosBuilder's new project template"
...
namespace :package do
    ...
    task :app => :default do
        ...
        built_files = `find . -name CocosBuilder.app`.chomp.split "\n"
        ...
        Dir.chdir BUILD_DIR do
            ...
            `zip -q -r CocosBuilder.app.dSYM.zip CocosBuilder.app.dSYM`
        end
    end

    desc "Create CocosBuilder.xcarchive and zip"
    task :archive do
        call_xctool "TARGET_BUILD_DIR=#{BUILD_DIR} SB_VERSION='#{DEFAULT_SB_VERSION}' VERSION='#{DEFAULT_SB_VERSION}' -configuration Release archive -archivePath #{BUILD_DIR}/CocosBuilder"

        Dir.chdir BUILD_DIR do
            sh "zip -r CocosBuilder.xcarchive.zip CocosBuilder.xcarchive"
        end
    end
end

desc "Build CocosBuilder distribution"
